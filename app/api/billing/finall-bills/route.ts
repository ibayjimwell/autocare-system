...
// --------------------------------------------------------------
// GET /api/billing/finall-bills?status=...&appointmentId=...
// --------------------------------------------------------------
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (appointmentId) query = query.where(eq(FinalBill.appointmentId, appointmentId));
    ...
    if (billIds.length > 0) {
      ...
      const discounts = await Database.select()
        ...
        .where(sql`${FinalBillDiscounts.finalBillId} IN (${billIds.join(',')})`);
      ...
      const tasks = await Database.select()
        ...
        .where(sql`${FinalBillWorkTasks.finalBillId} IN (${billIds.join(',')})`);
