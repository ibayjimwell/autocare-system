...
// ---------------------------------------------------------------
// GET /api/billing/finall-bills/:id
// ---------------------------------------------------------------
  ...
  try {
    ...
    if (findingIds.length > 0) {
      const parts = await Database.select()
        ...
        .where(eq(FinalBillFindingParts.finalBillFindingId, sql`ANY(${findingIds})`));
      for (const p of parts) {
        if (!partsMap[p.finalBillFindingId]) partsMap[p.finalBillFindingId] = [];
