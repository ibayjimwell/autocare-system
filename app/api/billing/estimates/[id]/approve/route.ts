...
// --------------------------------------------------------------------
// PATCH /api/billing/estimates/:id/approve
// --------------------------------------------------------------------
  ...
  try {
    ...
    if (estimate.status !== 'WAITING_FOR_APPROVAL') {
      return NextResponse.json({
        ...
        errorMessage: "Only estimates in WAITING_FOR_APPROVAL can be approved.",
        errorLog: null,
      }, { status: 422 });
    }
    ...
    console.error("[PATCH /api/service-tracking/estimates/[id]/approve] Error:", e);
    return NextResponse.json({
      error: true,
      errorType: "dbe",
