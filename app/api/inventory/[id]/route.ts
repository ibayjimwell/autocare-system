...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isValidUUID(id)) {
    return NextResponse.json({ error: true, errorType: "fve", errorTitle: "Invalid ID", errorMessage: "ID must be a valid UUID." }, { status: 422 });
  }
  let body: any;
  ...
  if (body.lowStockAlert !== undefined) updateData.lowStockAlert = body.lowStockAlert;
  if (body.active !== undefined) updateData.active = body.active;
  updateData.updatedAt = new Date();

  try {
    const [updated] = await Database.update(Inventory).set(updateData).where(eq(Inventory.id, id)).returning();
    if (!updated) {
    ...
    return NextResponse.json({ error: false, message: "Inventory item updated.", data: updated }, { status: 200 });
  } catch (e) {
    console.error("[PUT /api/inventory/[id]] Error:", e);
    return NextResponse.json({ error: true, errorType: "dbe", errorTitle: "Database update error", errorMessage: "Could not update inventory item." }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: false, message: "Inventory item deleted." }, { status: 200 });
  } catch (e) {
    console.error("[DELETE /api/inventory/[id]] Error:", e);
    return NextResponse.json({ error: true, errorType: "dbe", errorTitle: "Database deletion error", errorMessage: "Could not delete inventory item." }, { status: 500 });
  }
}