...
export const finalBillsApi = {
  ...
  list: async (params?: { status?: string; appointmentId?: string }) => {
    ...
    const url = qs ? `/api/billing/final-bills?${qs}` : '/api/billing/final-bills';
  ...
  get: async (id: string) => {
    const res = await fetch(`/api/billing/final-bills/${id}`);
    return res.json();
  },
  ...
  toggleFinding: async (billId: string, findingId: string) => {
    const res = await fetch(`/api/billing/final-bills/${billId}/findings/${findingId}/toggle`, {
      method: 'PATCH',
    });
    return res.json();
  },
  ...
  toggleFee: async (billId: string, feeId: string) => {
    const res = await fetch(`/api/billing/final-bills/${billId}/fees/${feeId}/toggle`, {
      method: 'PATCH',
    });
    return res.json();
  },
