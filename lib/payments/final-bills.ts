...
export const finalBillsApi = {
  ...
  toggleFinding: async (billId: string, findingId: string) => {
    const res = await fetch(`/api/payments/final-bills/${billId}/findings/${findingId}/toggle`, {
      method: 'PATCH',
    });
    return res.json();
  },
  ...
  toggleFee: async (billId: string, feeId: string) => {
    const res = await fetch(`/api/payments/final-bills/${billId}/fees/${feeId}/toggle`, {
      method: 'PATCH',
    });
    return res.json();
  },
