...
export default function FindingModal({ open, onClose, appointmentId, onSaved }: FindingModalProps) {
  ...
  const handleInventorySelect = (findingId: string, partId: string, item: { name: string; price: number; quantity: number }) => {
    setFindings(findings.map(f => {
      if (f.id === findingId) {
        return {
          ...f,
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[700px] p-0 flex flex-col h-[90vh]">
                    ...
                    <div key={part.id} className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end bg-muted/30 p-3 rounded-lg relative">
                      <div className="col-span-1">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          />
                          <InventoryPicker
                            onSelect={(item) => handleInventorySelect(finding.id, part.id, item)}
                            className="h-9 px-2"
                          >
                            <Package className="w-4 h-4" />
                            <span className="sr-only md:not-sr-only md:inline-block ml-1 text-xs">Pick</span>
                          </InventoryPicker>
                        </div>
                      </div>
                      <div className="col-span-1">
